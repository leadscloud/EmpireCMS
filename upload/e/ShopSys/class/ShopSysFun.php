...
function EditBuycar($add){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			if($del[$j]==$productid[$i]&&$delatt[$j]==$addatt[$i])
			{
				$isdel=1;
				break;
			}
...
function ShopSys_GetPre($precode,$totalmoney,$user,$classids){
	...
	$prer=$empire->fetch1("select id,prename,precode,premoney,pretype,reuse,endtime,groupid,classid,musttotal from {$dbtbpre}enewsshop_precode where precode='$precode' limit 1");
	if(!$prer['id'])
	{
		printerror("EmptyPreCode","history.go(-1)",1);
	}
...
function AddDd($add){
	...
	//优惠码
	if($prer['id']&&$prer['reuse']==0)
	{
		$empire->query("delete from {$dbtbpre}enewsshop_precode where id='".intval($prer['id'])."'");
	}
	if($sql)
	{
