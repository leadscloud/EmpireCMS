...
function AddZtF($add,$userid,$username){
	...
	if($asql&&$sql)
	{
		...
		printerror("AddFSuccess","special/AddZtF.php?enews=AddZtF");
	}
	else
	{
...
function AddZt($add,$userid,$username){
	...
	if($sql){
		...
		printerror("AddZtSuccess","special/AddZt.php?enews=AddZt");
	}
	else{
		printerror("DbError","");
	}
...
function EditZt($add,$userid,$username){
	...
	if($sql)
	{
		...
		if($add['from'])
		{
			$returnurl='special/AddZt.php?enews=EditZt&ztid='.$add[ztid].'&from=1';
		}
...
function AddWbClass($add,$userid,$username){
	...
	if($sql)
	{
		...
		$cache_enews='doclass';
		$cache_ecmstourl=urlencode("AddClass.php?enews=AddClass&from=$add[from]");
...
function AddClass($add,$userid,$username){
	...
	if(!$add[islast])
	{
		...
		if($sql)
		{
			...
			$cache_mess='AddClassSuccess';
			$cache_mid=$add[modid];
			$cache_url="CreateCache.php?enews=$cache_enews&mid=$cache_mid&ecmstourl=$cache_ecmstourl&mess=$cache_mess";
			insert_dolog("classid=".$lastid."<br>classname=".$add[classname]);//操作日志
			//printerror("AddClassSuccess","AddClass.php?enews=AddClass&from=$add[from]");
			echo'<meta http-equiv="refresh" content="0;url='.$cache_url.'">';
			db_close();
...
function EditClass($add,$userid,$username){
	...
	if($sql)
	{
		...
		$cache_ecmstourl=urlencode($returnurl);
		$cache_mess='EditClassSuccess';
		$cache_url="CreateCache.php?enews=$cache_enews&mid=$cache_mid&oldmid=$cache_oldmid&ecmstourl=$cache_ecmstourl&mess=$cache_mess";
		//printerror("EditClassSuccess",$returnurl);
		echo'<meta http-equiv="refresh" content="0;url='.$cache_url.'">';
		db_close();
		$empire=null;
