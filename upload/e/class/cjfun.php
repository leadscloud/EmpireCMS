...
function EditCjNews($add,$newstext,$userid,$username){
	...
	if($sql)
	{
		...
		printerror("EditCjNewsSuccess","CheckCj.php?classid=$add[classid]&from=$_POST[from]");
	}
	else
...
function DelCjNews($classid,$id,$userid,$username){
	...
	if($sql)
	{
		...
		printerror("DelCjNewsSuccess","CheckCj.php?classid=$classid&from=$_GET[from]");
	}
	else
	{printerror("DbError","history.go(-1)");}
}
...
function PageEchoUrl($classid,$cr,$userid,$username){
	...
	echo $fun_r['GetUrlOver']."<script>self.location.href='ecmscj.php?enews=GetNewsInfo&classid=$classid&start=0&checkrnd=$checkrnd';</script>";
	exit();
}
...
function CJUrl($classid,$start,$checkrnd,$userid,$username){
	...
	//echo $fun_r['GetOneListUrl']."<script>self.location.href='ecmscj.php?enews=CjUrl&classid=$classid&start=$newstart&checkrnd=$checkrnd';</script>";
	echo"<meta http-equiv=\"refresh\" content=\"".$r['keeptime'].";url=ecmscj.php?enews=CjUrl&classid=$classid&start=$newstart&checkrnd=$checkrnd\">".$fun_r['GetOneListUrl'];
	exit();
}
...
function ViewCjList($classid,$userid,$username){
	...
	for($i=0;$i<$j;$i++)
	{
		...
		$data.="<tr><td><a href='".$url_r[$i]."' target=_blank>".$url_r[$i]."</a></td><td align=center>[<a href='ecmscj.php?enews=ViewCjUrl&classid=$classid&listpage=".$dourl."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
    }
...
function PageViewCjList($classid,$cr,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$data.="<tr><td><a href='".$url_r[$i]."' target=_blank>".$url_r[$i]."</a></td><td align=center>[<a href='ecmscj.php?enews=ViewGetNewsInfo&classid=$classid&newspage=".$dourl."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
    }
	$data="<p align=center><b>".$fun_r['CjListUrl']."</b></p><table width='96%' border=1 align=center cellpadding=3 cellspacing=0>
  <tr><td width=70% align=center><b>URL</b></td><td align=center><b>VIEW</b></td></tr>".$data."</table>";
	echo $data;
...
function ViewEchoUrl($text,$exp,$exp1,$dr,$url,$classid,$num,$checkrnd){
	...
	for($i=1;$i<count($r)&&$i<=$num;$i++)
	{
		...
		echo "<tr><td><a href='".$dourl."' target=_blank>".$dourl."</a>".$a."</td><td align=center>[<a href='ecmscj.php?enews=ViewGetNewsInfo&classid=$classid&newspage=".urlencode($dourl)."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
	}
}
...
function GetNewsInfo($classid,$checkrnd,$start,$userid,$username){
	...
	{printerror("FailCX","ListInfoClass.php");}
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	...
	if(empty($b))
	{
		...
		if($r['justloadin'])
		{
			echo $fun_r['cjLoadInInfos']."<script>self.location.href='ecmscj.php?enews=CjNewsIn_all&classid=$classid&checked=$r[justloadcheck]&fm=1';</script>";
			exit();
		}
		//单个节点
		if(getcvar('recjnum',1)==1)
		{
			echo"<link rel=\"stylesheet\" href=\"../data/images/css.css\" type=\"text/css\"><font color=red>".$fun_r['CjSuccess']."</font><script>parent.location.href='CheckCj.php?classid=$classid';</script>";
		}
		else
		{
			echo"<link rel=\"stylesheet\" href=\"../data/images/css.css\" type=\"text/css\"><body topmargin=0><font color=red>".$r[classname]."  ".$fun_r['CjSuccess']."</font>,  <input type=button name=button value='".$fun_r['OnlickLoadInCj']."' onclick=\"window.open('CheckCj.php?classid=$classid');\">";
			echo"<script>parent.checkrecj.location.href='CheckReCj.php';</script></body>";
		}
		exit();
	}
	//echo"(ID:<font color=red><b>".$newstart."</b></font>)<script>self.location.href='ecmscj.php?enews=GetNewsInfo&checkrnd=$checkrnd&classid=$classid&start=$newstart';</script>";
	echo"<meta http-equiv=\"refresh\" content=\"".$r['keeptime'].";url=ecmscj.php?enews=GetNewsInfo&checkrnd=$checkrnd&classid=$classid&start=$newstart\">".$r[classname]." (ID:<font color=red><b>".$newstart."</b></font>)";
	exit();
}
...
function LoadInCj($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	printerror("LoadInCjSuccess","cj/LoadInCj.php?from=$_POST[from]");
}
